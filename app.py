...
def grade_citation(ref, citation):
    ...
    if is_correct:
        ...
        feedback = "일부 요소는 맞았지만 아래 오류를 다시 확인해보세요."
    else:
...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS submissions (
            id TEXT PRIMARY KEY,
            student_class TEXT, student_id TEXT, student_name TEXT,
            q1_citation TEXT, q1_score INTEGER, q1_correct INTEGER, q1_errors TEXT,
            q2_citation TEXT, q2_score INTEGER, q2_correct INTEGER, q2_errors TEXT,
            q3_citation TEXT, q3_score INTEGER, q3_correct INTEGER, q3_errors TEXT,
            q4_citation TEXT, q4_score INTEGER, q4_correct INTEGER, q4_errors TEXT,
            total_score INTEGER, timestamp TEXT
        )
    """)
    conn.commit()
    conn.close()
...
def save_submission(sub):
    ...
    c.execute(
        ...
        (
            sub["id"], sub["student_class"], sub["student_id"], sub["student_name"],
...
<div class="question-format">{ref["format_hint"]}</div>''', unsafe_allow_html=True)
    ...
    mc5.metric("3·4번 평균",   f"{int((avg_q3+avg_q4)/2)}점")
    ...
    student_options = filtered.apply(
        lambda r: f"{r['student_class']} / {r['student_id']} / {r['student_name']}", axis=1
    ).tolist()
